...
def log_results(out_file, msg, tw_cv, blog_cv, featurizer):
    ...
    if tw_cv and blog_cv:
        ...
        results_twb = experiment_linear_svm(tw_cv, blog_cv, featurizer)
...
def error_analysis(featurizers = None):
    ...
    blog_errors = error_analyze(Models.LinearSVC, b_train, b_test, featurizers[0])
    twb_errors = error_analyze(Models.LinearSVC, tw, blog_80, featurizers[1])
    tw_train, tw_test = train_test_split(tw, test_size = 0.1, random_state = 1)
    tw_errors = error_analyze(Models.LinearSVC, tw_train, tw_test, featurizers[2])
...
def tuning_l2_penalty(out_file, featurizers = None):
    ...
    for c_val in c_vals:
        ...
        make_model = lambda: Models.LinearSVC(C = c_val)
...
def test_perf(out_file):
    ...
    results_b = experiment_linear_svm(blog_80, blog_20, feat4)
    results_twb = experiment_linear_svm(tw, blog_20, feat5)
    results_tw = experiment_linear_svm(tw, twitter_test, feat4)
